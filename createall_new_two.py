...
def create_test_script(headers,auth,base_file_name,method, url, parameter=None,baseurl = 'https://petstore3.swagger.io/api/v3'):     
    ...
    try:                                   #exception handeling
        ...
        with open(file.name, 'w') as f:    #open(file.name, w) is for writing things into the file
            ...
            f.write("    resp=restapicalls.api_call(method,url,parameter,headers=headers,auth=auth)\n")
            f.write("    print(resp)\n")
            f.write(codebot)
...
def create_cucumber_script(baseurl,base_file_name,urls,methods,parameter_values,parameters,headers,auth):
    try:                                   #exception handeling
        file = createfile(typefile = 'py',base_file_name=base_file_name)
        txt = testcucumber.format(baseurl=baseurl,method=methods,url = urls, parameter_value= parameter_values, parameter = parameters,headers=headers,auth=auth)  
        #print (testcucumber)  
        with open(file.name, 'w') as f:    #open(file.name, w) is for writing things into the file
            f.write (txt) 
...
def add_cucumber_script(baseurl,base_file_name,urls,methods,parameter_values,parameters,headers,auth):
    try:                                   #exception handeling
        
        txt = addtestcucumber.format(baseurl=baseurl,method=methods,url = urls, parameter_value= parameter_values, parameter = parameters,headers=headers,auth=auth)  
...
def add_feature_script(file,featurename, scenarioname, url, pointingattribute, methods):    
    ...
    try:                                   #exception handeling
        ...
        with open(file, 'a') as f:    #open(file.name, w) is for writing things into the file #it will append all these into the new file
            ...
            f.write("\t"+"Given I set " + methods +' ' + url + " API url" +"\n")

            f.write("\t"+"And I have valid " + pointingattribute +"\n")

            f.write("\t"+"When I send "+ methods +" HTTPS request "+url+"\n")

            f.write("\t"+"Then I receive a valid HTTPS response code 200 from "+url+" using " + methods+"\n")  
...
def create_feature_script(file,featurename, scenarioname, url, pointingattribute, methods):    
...
def create_file(featuresdir='features/',baseurl=None,typescript=None,projectid= None,userstoryid=None,method=None,url=None,parameter=None,parameter_values=None,featurename=None,scenarioname=None,pointingattribute=None,headers=None,auth=None):
    if typescript == 'testscript':           
        base_file_name = "testAPI"+projectid+userstoryid
        create_test_script(headers=headers,auth=auth,baseurl=baseurl,base_file_name=base_file_name,method=method, url= url, parameter=parameter)
    if typescript == 'stepsscript':
        base_file_name =featuresdir+"steps/"+"testcucumber"+projectid+userstoryid
        create_cucumber_script(baseurl=baseurl,base_file_name= base_file_name,urls=url,methods=method,parameter_values=parameter_values,parameters=parameter,headers=headers,auth=auth)
    ...
    if typescript == 'createfeature':                            #conditionals
        ...
        create_feature_script(file=newfile,featurename=featurename, scenarioname=scenarioname, url=url, pointingattribute=pointingattribute, methods=method)
    if typescript == 'addtofeature':                            #conditionals
        ...
        add_feature_script(file=base_file_name,featurename=featurename, scenarioname=scenarioname, url=url, pointingattribute=pointingattribute, methods=method)  
    if typescript=='addtostepsscript':
        base_file_name =featuresdir+"steps/"+"testcucumber"+projectid+userstoryid+'auto.py'
        add_cucumber_script(baseurl=baseurl,base_file_name= base_file_name,urls=url,methods=method,parameter_values=parameter_values,parameters=parameter,headers=headers,auth=auth)
       
def create_file_new(typescript='testscript',projectid= "1111",userstoryid="2222",method=None,url=None,parameter=None,contenttype=None, auth_type=None, parameter_values=None,featurename=None,scenarioname=None,pointingattribute=None,headers=None,auth=None,featuresdir='features/'):
#def create_file_new(featuresdir='features/',contenttype=None,baseurl=None,typescript=None,projectid= None,userstoryid=None,method=None,endpoint=None,parameter=None,parameter_values=None,featurename=None,scenarioname=None,pointingattribute=None,headers=None,auth=None,auth_type=None):
    ...
    if auth_type=='headers':
        ...
        print("Be Patient, Time will give you all the success your heart desires")
        auth = "HTTPDigestAuth" + auth 
    elif auth == "HTTPBasicAuth":
        auth = "HTTPBasicAuth" + auth  
    create_file(featuresdir=featuresdir,baseurl=baseurl,typescript=typescript,projectid= projectid,userstoryid=userstoryid,method=method,url=url,parameter=parameter,parameter_values=parameter_values,featurename=featurename,scenarioname=scenarioname,pointingattribute=pointingattribute,headers=headers,auth=auth)    
...
#create_file(baseurl=baseurl3,typescript='addtostepsscript',projectid='peet',userstoryid='storee',featurename = 'testing github API authentication', scenarioname ='API authentication for github' ,endpoint =endpoint3 , pointingattribute = 'githubuserid2', method='GET',parameter={'type':'all'},parameter_values='githubuserid2', headers=githubheader)
#create_file_new(baseurl=baseurl2,contenttype='xml',typescript='stepsscript',projectid='peet',userstoryid='storee',featurename = 'testing httpbin API authentication', scenarioname ='API authentication for httpbin' ,endpoint =endpoint2 , pointingattribute = 'userid', method='GET',parameter=None,parameter_values='userid',auth="('user', 'pass')",auth_type='HTTPDigestAuth')
